...
describe('putDoc', () => {
  ...
  it('syncToRemote: online success aktualisiert _rev und _dirty=false', async () => {
    ...
    globalThis.fetch = vi.fn()
      .mockResolvedValueOnce(jsonOk({ _id: 'sync_ok', _rev: '2-server', type: 'item', name: 'Test' }))
  ...
  it('syncToRemote: Server-Fehler (z.B. 500) wird still ignoriert', async () => {
    ...
    const { putDoc } = await loadDB()
    await expect(putDoc({ _id: 's500', type: 'item', name: 'Test' })).resolves.toMatchObject({ ok: true })
    await new Promise(r => setTimeout(r, 50))
  })
})
...
describe('clearRemoteChangedFlag', () => {
  it('entfernt _remoteChanged und _changeTimestamp', async () => {
    ...
    await putDoc({ _id: 'flag_1', type: 'item', name: 'Test', _remoteChanged: true, _changeTimestamp: 12345 })
...
describe('restoreLocalVersion', () => {
  ...
  it('gibt false zurück wenn putDoc fehlschlägt', async () => {
    ...
    const result = await restoreLocalVersion('unknown_id', {
      _id: 'unknown_id', type: 'item', name: 'Test'
    })
...
describe('startSync / stopSync', () => {
  ...
  it('syncInBackground ruft statusCallback mit online=true auf wenn Server erreichbar', async () => {
    // _changes feed → ok
    globalThis.fetch = vi.fn()
      .mockResolvedValue(jsonOk({ results: [] }))
...
describe('syncFromRemote (via startSync)', () => {
  it('neues Dokument vom Remote wird lokal gespeichert', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(jsonOk({
      results: [
        { doc: { _id: 'remote_1', _rev: '1-a', type: 'item', name: 'Remote Item' } }
      ]
    }))
  ...
  it('remote gelöschtes Dokument (deleted=true) wird lokal entfernt', async () => {
    const { putDoc, startSync, getDoc } = await loadDB()
    // Erst lokal anlegen
    await putDoc({ _id: 'to_del', type: 'item', name: 'Wird gelöscht', _rev: '1-a' })

    globalThis.fetch = vi.fn().mockResolvedValue(jsonOk({
      results: [
        { deleted: true, doc: { _id: 'to_del', _rev: '2-b', _deleted: true } }
      ]
    }))
  ...
  it('remote Dokument mit markedDeleted=true setzt _pendingDelete lokal', async () => {
    ...
    await db2.putDoc({ _id: 'pd_item2', type: 'item', name: 'PD Test', _rev: '1-a', markedDeleted: false })

    globalThis.fetch = vi.fn().mockResolvedValue(jsonOk({
      results: [
        {
          doc: {
            _id: 'pd_item2', _rev: '2-b',
            type: 'item', name: 'PD Test',
            markedDeleted: true,
  ...
  it('dataChangeCallback wird aufgerufen wenn es Änderungen gibt', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(jsonOk({
      results: [
        { doc: { _id: 'change_1', _rev: '1-a', type: 'item', name: 'Neu' } }
      ]
    }))
...
describe('resolveConflict (via syncToRemote 409)', () => {
  it('Fall 1: Remote markedDeleted=true → _pendingDelete wird gesetzt', async () => {
    ...
    globalThis.fetch = vi.fn()
      ...
      .mockResolvedValueOnce(jsonOk({
        ...
        markedDeleted: true,
        lastModifiedBy: 'Bob',
        type: 'item', name: 'Remote gelöscht'
      }))
  ...
  it('Fall 2: Lokal markedDeleted=true, remote nicht → lokal gewinnt', async () => {
    ...
    globalThis.fetch = vi.fn()
      ...
      .mockResolvedValueOnce(jsonFail(409))
      // resolveConflict GET
      .mockResolvedValueOnce(jsonOk({ _id: 'cf2', _rev: '3-r', markedDeleted: false, type: 'item', name: 'Remote' }))
      // resolveConflict PUT
  ...
  it('Fall 3: Auto-merge upload schlägt fehl → false', async () => {
    ...
    globalThis.fetch = vi.fn()
      ...
      .mockResolvedValueOnce(jsonFail(409))
      .mockResolvedValueOnce(jsonOk({ _id: 'cf4', _rev: '3-r', markedDeleted: false, type: 'item', name: 'Remote', fieldTimestamps: {} }))
      .mockResolvedValueOnce(jsonFail(500, 'Server Error'))

    await db2.putDoc({ _id: 'cf4', type: 'item', name: 'Update', markedDeleted: false, fieldTimestamps: { name: '2025-01-01T00:00:00.000Z' } })
    await new Promise(r => setTimeout(r, 100))
    // kein Fehler-Throw erwartet
  })
})
  ...
  it('wirft wenn IDB delete einen Fehler liefert', async () => {
    ...
    const fakeDB = {
      ...
      transaction: () => ({
        ...
        objectStore: () => ({
          delete: () => {
            const req = { result: undefined, error: new Error('IDB delete error'), onsuccess: null, onerror: null }
    ...
    const fakeDB = {
      ...
      transaction: (storeName, mode) => {
        ...
        return {
          ...
          objectStore: () => ({
            get: (id) => {
              const req = { result: { _id: id, _rev: '1-a' }, error: null, onsuccess: null, onerror: null }
              Promise.resolve().then(() => req.onsuccess?.({ target: req }))
              return req
            },
            getAll: () => {
...
describe('applyConflictResolution', () => {
  it('gibt false zurück wenn Server nicht erreichbar (offline)', async () => {
    ...
    const result = await applyConflictResolution('doc_1', { _id: 'doc_1', _rev: '1-a', type: 'item' })
    expect(result).toBe(false)
  })

  it('gibt true zurück wenn Server antwortet', async () => {
    globalThis.fetch = vi.fn()
      .mockResolvedValueOnce(jsonOk({ _id: 'doc_1', _rev: '2-server', type: 'item', name: 'Remote' }))
      .mockResolvedValueOnce(jsonOk({ ok: true, id: 'doc_1', rev: '3-new' }))

    const { applyConflictResolution } = await loadDB()
    const result = await applyConflictResolution('doc_1', {
      _id: 'doc_1', _rev: '2-server', type: 'item', name: 'Chosen',
    })
    expect(result).toBe(true)
  })
  ...
  it('gibt false zurück wenn PUT fehlschlägt', async () => {
    ...
    const { applyConflictResolution } = await loadDB()
    const result = await applyConflictResolution('doc_2', { _id: 'doc_2', _rev: '1-a', type: 'item' })
    expect(result).toBe(false)
  })
  ...
  it('GET schlägt fehl → verwendet _rev aus chosenDoc', async () => {
    ...
    const { applyConflictResolution } = await loadDB()
    const result = await applyConflictResolution('doc_3', { _id: 'doc_3', _rev: '1-a', type: 'item', name: 'Test' })
    expect(result).toBe(true)
  })
})
